...
usage() {
  cat <<'EOF'
Usage:
  pull_session.sh --session SESSION [--remote-root ~/pilot] [--local-root PATH]
                  [--include-logs] host1 [host2 ...]

Example:
  pull_session.sh --session csi_single_walk_01 rpi-csi-rx-a rpi-csi-rx-b rpi-lav-audio
EOF
}
