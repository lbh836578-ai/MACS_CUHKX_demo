...
usage() {
  cat <<'EOF'
Usage:
  start_csi_rx.sh --session SESSION [--channel 36/40] [--interface wlan0]
                  [--src-macs aa:bb:cc:dd:ee:ff,11:22:33:44:55:66]
                  [--outdir PATH] [--packets 1000]

Notes:
  - Defaults assume nexmon_csi is installed at ~/nexmon/patches/bcm43455c0/7_45_189/nexmon_csi
  - If --src-macs is set, only matching transmitter MACs are collected at firmware level
...
if ! sudo -n tcpdump --version >/dev/null 2>&1; then
  log "CSI capture requires passwordless sudo for tcpdump, nexutil, nmcli, ip, rm, and kill."
  exit 1
fi
...
cd "$NEXMON_CSI_DIR"
PARAM_ARGS=(-c "$CHANNEL" -C 1 -N 1)
if [[ -n "$SRC_MACS" ]]; then
  PARAM_ARGS+=(-m "$SRC_MACS")
fi
...
log "Configuring CSI capture on $INTERFACE with channel $CHANNEL src_macs=${SRC_MACS:-all}"
