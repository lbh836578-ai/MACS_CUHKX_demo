...
class MainWindow(QMainWindow):
    ...
    recording_started   = pyqtSignal(list)              # label list
    ...
    @pyqtSlot(list)
    def _on_start(self, labels):
        if self._state != AppState.IDLE or not labels:
            return
        ...
        self.recording_started.emit(labels)
    ...
    def on_post_processing_done(self, output_dir=None, error=None):
        """Called when the PostProcessor finishes."""
        self.setWindowTitle("MACS - MultiModal Action Capture System")
    ...
    def keyPressEvent(self, event):
        ...
        if (self._state == AppState.IDLE
                and self.control_panel.label_input.hasFocus()):
        ...
        if key == Qt.Key_Space and self._state == AppState.IDLE:
            labels = self.control_panel.get_labels()
            if labels:
                self._on_start(labels)
