...
class PostProcessor(QThread):
    ...
    def _resolve_output_dir(self, session_name):
        rec_cfg = self._cfg.get("recording", {})
        raw_sub = rec_cfg.get("raw_subdir", "raw")
        proc_sub = rec_cfg.get("processed_subdir", "processed")

        if self._session_dir.parent.name == raw_sub:
            base_dir = self._session_dir.parent.parent
        else:
            base_dir = Path(rec_cfg.get("output_dir", "data"))

        return base_dir / proc_sub / session_name
